...
/**
 * Validate an EDS block name (alphanumeric + single hyphens).
 * @param {string} blockName
 * @returns {{ valid: true } | { valid: false, error: string }}
 */
function validateBlockName(blockName) {
  if (!blockName || !BLOCK_NAME_PATTERN.test(blockName)) {
    return {
      valid: false,
      error: `Invalid block name "${blockName}". Block names must use lowercase letters, numbers, and single hyphens only.`,
    };
  }
  return { valid: true };
}
...
/**
 * Find a block element by class token without building CSS selectors from user input.
 * @param {ParentNode} root
 * @param {string} blockName
 * @returns {Element|null}
 */
function findBlockByName(root, blockName) {
  return Array.from(root.querySelectorAll('div[class]')).find(
    (element) => element.classList.contains(blockName),
  ) ?? null;
}
...
function analyzeStructure(html, blockName) {
  ...
  const block = findBlockByName(doc, blockName);
  ...
  try {
    ...
    sections.forEach(section => {
      ...
      if (metadata) {
        ...
        // Find the block content (div whose class list includes the block name)
        const blockDiv = findBlockByName(section, blockName);
