...
- **content-modeling**: Provides the Author View from Step 3; repeat it when implementation completes
...
- [ ] Step 6: Deliver Author View in chat response (required for new blocks and structure changes)
...
## Step 6: Deliver Author View (Required)

**When:** Every **new block** or change that affects **author-facing structure**.
**Skip when:** CSS-only tweaks, bug fixes, or decoration changes that do not
change what authors create.

Authors cannot create content without knowing the Google Docs / DA table layout.
Include the Author View **in the same chat response** as the implementation —
never defer to a follow-up.

**What to do:**

1. Reproduce the **Author View** from content-modeling Step 3 (update if
   implementation changed structure).
2. If no Author View exists, generate one using
   [content-modeling/references/author-view-guide.md](../content-modeling/references/author-view-guide.md).
3. **Recommended:** persist to `blocks/{block-name}/AUTHORING.md` for new blocks.

**The Author View must include:**
- Where content goes (intro above table vs. inside block)
- Full table layout with **realistic example values** (not "Cell 1" placeholders)
- Column/row rules and formatting (headings, bold, italic, links)
- Do / Don't table
- What authors control vs. what code handles

**This is a required deliverable alongside code files.** Do not mark
implementation complete without it.
