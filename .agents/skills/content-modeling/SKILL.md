...
- **da-content**: Load for Google Docs / DA authoring rules when writing the Author View
...
- [ ] Step 4: Document and return content model **with Author View**. See "Step 4: Document and Return" below
...
**The Author View section is mandatory** — authors need it immediately to create
Google Docs / DA content. Do not defer it to a later step or a follow-up message.

Read [references/author-view-guide.md](references/author-view-guide.md) for the
full template, canonical-model patterns, and Google Docs / DA conventions.
...
### Where content goes
- **Section intro:** [what lives above the block table]
- **Block table:** [one table in Google Docs / DA]

### Table layout (example values)
[Show every row with realistic example content — not abstract "Cell 1" labels.
Use markdown or ASCII table. This is what authors copy into Google Docs.]
...
### Optional variants
- `[Block Name (variant)]` — [effect]
```

**Important:** Return the full output (Content Model **and** Author View) to the
calling skill or user. building-blocks will repeat the Author View when
implementation completes and may persist it to `blocks/{block-name}/AUTHORING.md`.
...
### [references/author-view-guide.md](references/author-view-guide.md)
Required template for the **Author View** deliverable — generic patterns for all
canonical block models (Standalone, Collection, Configuration, Auto-Blocked),
Google Docs / DA conventions, and anti-patterns. Block-specific examples belong
in `blocks/{block-name}/AUTHORING.md`.
